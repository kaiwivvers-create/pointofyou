...
<div id="bulkPurchaseModal" class="fixed inset-0 bg-black/80 backdrop-blur-sm hidden items-center justify-center z-[9999] transition-opacity duration-200">
    <div class="bg-white rounded-lg shadow-xl max-w-3xl w-full mx-4 max-h-[90vh] overflow-y-auto transform transition-all duration-200 scale-95 opacity-0" id="bulkPurchaseModalContent">
        <div class="p-6 border-b border-slate-200 flex justify-between items-start gap-4">
            <div>
                <h2 class="text-xl font-semibold text-slate-900"><?php echo e($bulkPurchaseTitle); ?></h2>
                <p class="text-sm text-slate-500 mt-1"><?php echo e($bulkPurchaseDescription); ?></p>
            </div>
        ...
        <form method="POST" action="<?php echo e(route('inventory.bulk-purchases.store')); ?>" class="p-6 space-y-4">
            <?php echo csrf_field(); ?>
...
<script>
    const bulkPurchaseProductOptions = <?php echo json_encode($productOptions, 15, 512) ?>;
...
<?php /**PATH C:\Users\cindy\OneDrive\Documents\tick B\pplg\writing hting\pointofyou\pointofyou\resources\views/inventory/partials/bulk-purchase-modal.blade.php ENDPATH**/ ?>