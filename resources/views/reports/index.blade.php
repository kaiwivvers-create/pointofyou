...
@section('content')
    ...
    <div class="print-header">
        ...
        <p class="text-slate-600">Generated on {{ now()->format('F j, Y H:i') }}</p>
    ...
    <div class="staff-page-header no-print">
        ...
        <div class="flex gap-2 no-print">
            <a href="{{ route('reports.export.csv', request()->all()) }}" class="staff-btn-secondary no-print">Export CSV</a>
            <button onclick="window.open('{{ route('reports.print', request()->all()) }}', '_blank')" class="staff-btn-secondary no-print">Print PDF</button>
        </div>
    </div>
        ...
        <div class="flex flex-col sm:flex-row gap-4 items-center">
            <div class="flex-1">
                ...
                <select name="chart_period" class="staff-input" onchange="updateChart()">
                    <option value="year" {{ $chartPeriod === 'year' ? 'selected' : '' }}>Yearly (Monthly)</option>
                    <option value="month" {{ $chartPeriod === 'month' ? 'selected' : '' }}>Monthly (Daily)</option>
                </select>
            </div>
            <div class="flex-1">
    ...
    <div class="staff-card p-6 mb-6 chart-section no-print">
        <h2 class="text-lg font-semibold text-slate-900 mb-4">
            {{ $chartPeriod === 'year' ? 'Monthly Income (This Year)' : 'Daily Income (This Month)' }}
        </h2>
        <div class="h-80">
    ...
    <!-- Date Filter -->
    <div class="staff-card p-6 mb-6 no-print">
        <form method="GET" action="{{ route('reports.index') }}" class="flex flex-col sm:flex-row gap-4 no-print">
            <div class="flex-1">
            ...
            <div id="customDates" class="flex gap-4 {{ $period !== 'custom' ? 'hidden' : '' }}">
                <div class="flex-1">
                    <label class="block text-sm font-medium text-slate-700 mb-1">Start Date</label>
                    <input type="date" name="start_date" value="{{ $startDate ?? '' }}" class="staff-input">
                </div>
                <div class="flex-1">
                    <label class="block text-sm font-medium text-slate-700 mb-1">End Date</label>
                    <input type="date" name="end_date" value="{{ $endDate ?? '' }}" class="staff-input">
                </div>
            </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        ...
        <div class="staff-card p-6">
            ...
            <p class="text-3xl font-bold text-red-600">${{ number_format($totalOutcome, 2) }}</p>
        </div>
        <div class="staff-card p-6">
            <p class="text-sm text-slate-500 mb-1">Net Profit</p>
            <p class="text-3xl font-bold {{ $netProfit >= 0 ? 'text-slate-900' : 'text-red-600' }}">${{ number_format($netProfit, 2) }}</p>
        </div>
        <div class="staff-card p-6">
                    ...
                    <tbody>
                        @forelse ($orders as $order)
                            <tr>
                                ...
                                <td class="text-right font-semibold text-emerald-600">${{ number_format($order->total, 2) }}</td>
                            </tr>
                        @empty
                            <tr>
