...
@section('content')
    <div class="staff-page-header">
        <div>
            <h1 class="staff-page-title">Expense Categories</h1>
            <p class="staff-page-subtitle">Organize expenses into categories.</p>
        </div>
                ...
                <tbody>
                    @forelse ($categories as $category)
                        <tr>
                            ...
                            <td>
                                @if($category->color)
                                    <span class="px-2 py-1 rounded text-xs text-white" style="background-color: {{ $category->color }};">{{ $category->color }}</span>
                                @else
                                    <span class="text-slate-400">-</span>
                                @endif
                            </td>
                            <td class="text-right space-x-4">
                                <span class="text-slate-400 text-sm">{{ $category->expenses()->count() }} expenses</span>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="py-16 text-center text-slate-500">No categories yet. Add your first category!</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div id="addCategoryModal" class="fixed inset-0 bg-black/80 backdrop-blur-sm hidden items-center justify-center z-[9999] transition-opacity duration-200">
        <div class="bg-white rounded-lg shadow-xl max-w-lg w-full mx-4 max-h-[90vh] overflow-y-auto transform transition-all duration-200 scale-95 opacity-0" id="addCategoryModalContent">
            ...
            <form method="POST" action="{{ route('expenses.categories.store') }}" class="p-6">
                @csrf
                <div class="space-y-4">
                    <div>
                    ...
                    <div>
                        ...
                        <input type="color" name="color" value="#6366f1" class="h-10 w-full rounded border border-slate-300">
                    </div>
                </div>
