        ...
        <form method="POST" action="{{ route('admin.gifts.update', $gift) }}" enctype="multipart/form-data">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    @if ($gift->image)
                        <p class="mt-2 text-sm text-slate-600">Current: <img src="{{ asset('storage/' . $gift->image) }}" alt="{{ $gift->name }}" class="inline-block w-16 h-16 object-cover rounded border border-slate-200"></p>
                    @endif
                </div>
