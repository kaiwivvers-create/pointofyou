            ...
            @forelse($cart as $index => $cartItem)
                <div class="bg-white p-4 rounded-2xl shadow-sm border border-stone-200 flex gap-3 relative hover:border-amber-300 transition-colors">
                    ...
                    <form method="POST" action="{{ route('kiosk.cart.remove', $index) }}" class="absolute top-3 right-3">
                        @csrf
                        <button type="submit" class="w-7 h-7 rounded-full bg-stone-100 text-stone-400 flex items-center justify-center text-sm hover:bg-red-500 hover:text-white hover:border-red-500 transition-colors border border-stone-200 cursor-pointer">×</button>
                    </form>
                </div>
            @empty
