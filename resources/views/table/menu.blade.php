            ...
            <div class="flex flex-col sm:flex-row gap-4">
                ...
                <div class="flex gap-2">
                    <select id="categoryFilter" onchange="filterMenuItems()" class="px-4 py-3 rounded-xl border border-stone-200 focus:border-amber-500 focus:ring-2 focus:ring-amber-200 transition-all outline-none bg-white">
                        ...
                        @foreach($itemsByCategory->keys() as $category)
                            <option value="{{ strtolower($category) }}">{{ $category === 'packets' ? 'Packets' : $category }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <div id="cat-{{ strtolower($category) }}" class="mb-14">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
                        @foreach($items as $item)
                            ...
                            @endphp
                            @if($isPacket)
                                <button onclick="openPacketModal({{ $item->toJson() }}, '{{ $img }}')" data-name="{{ $item->name }}" class="table-item-card item-card bg-white rounded-2xl sm:rounded-3xl shadow-sm border border-stone-200 flex flex-col text-left relative overflow-hidden group hover:shadow-xl hover:border-amber-300 cursor-pointer min-w-0">
                            ...
                            @else
                                <button onclick="openItemModal({{ $item->toJson() }}, '{{ $img }}')" data-name="{{ $item->name }}" class="table-item-card item-card bg-white rounded-2xl sm:rounded-3xl shadow-sm border border-stone-200 flex flex-col text-left relative overflow-hidden group hover:shadow-xl hover:border-amber-300 cursor-pointer min-w-0">
                                    <div class="w-full h-36 sm:h-48 bg-stone-100 overflow-hidden relative flex-shrink-0">
                                        <img src="{{ $img }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" alt="{{ $item->name }}">
                                        <div class="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                                    </div>
...
<script>
    ...
    window.filterMenuItems = function() {
        ...
        categoryDivs.forEach(div => {
            ...
            const itemCards = div.querySelectorAll('.table-item-card');
